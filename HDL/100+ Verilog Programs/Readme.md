...
# Testbenches
Testbenches are available for the following now, with rest on the way

## Level 1
All twenty modules of this level have a complete (with all possible cases) testbench in `level_1_tb.sv` inside the `Level / tb` folder.
Features:

 - All cases (ex: if a module has 4 bit input, all 2^4 = 16 cases are tested.)
 -  All 20 modules are tested at once in a single file
 - Total tested cases are shown
 - On Test case failure, cause of failure is shown
 
 

## Level 2
Excluding a few very small designs, rest have testbenches.
`counter.v` has a `.sv` testbench while others have, `cocotb` testbench.
>Note : The cocotb testbenches assumes a default simulator 'Verilator' to be installed and available on a global system call, such as on Path. You can use anyother cocotb-supported simulator by manually passing arguments when running the testbench

Features: 

 - Uses `cocotb` + `Verilator`, resulting fast to develop, but also fast to simulate testbenches
 - cocotb Debug support
